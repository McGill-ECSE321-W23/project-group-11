<template>
  ...
  export default {
    name: "payment",
    props: ['serviceDto','paymentAmount','email'],
    data() {
      return {
        userEmail: this.email || localStorage.getItem('email'),
        requestBody: this.serviceDto || JSON.parse(localStorage.getItem('serviceDto')), 
    ...
    methods: {
      submitPayment() {
        ...
        axiosClient.get('/payment/' + this.cardNumber + '').then(() => {
            axiosClient.post('/booking/', this.requestBody).then(() => {
                localStorage.removeItem('serviceDto'); 
